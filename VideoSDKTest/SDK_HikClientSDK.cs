...
namespace VideoSDKTest
{
    public class SDK_HikClientSDK
    {
        ...
        **************************************************/
        ...
        public const int MIN_SEGMENT_NUM = 3;//摄像机标定最小样本线数目        
                                             /**********************设备类型 end***********************/
        #endregion
        public delegate void fLoginResultCallBack(int lUserID,int dwResult, NET_DVR_DEVICEINFO_V30 lpDeviceInfo, IntPtr pUser);
        #region 海康结构体
    ...
    **************************************************/
        ...
        **********************************************************/
        ...
        Return:	
        **********************************************************/
        //public delegate void fVoiceDataCallBack(int lVoiceComHandle, string pRecvDataBuffer, uint dwBufSize, byte byAudioFlag, uint dwUser);
        ...
        **********************************************************/
        ...
        //语音转发
        [DllImport("HCNetSDK.dll")]
        public static extern int NET_DVR_StartVoiceCom_MR(int lUserID, fVoiceDataCallBack fVoiceDataCallBack, uint dwUser);
        ...
        **********************************************************/
        ...
        public static string _strDevType;
        private bool _isInit;//是否初始化
        private bool _isLogin;//是否登录服务器
        private bool _isPlaying;//是否正在播放
        private bool _isOpenSound;//是否打开声音
        private string _dvsNum;//DVS编号
        private int _dvsChannel;//DVS通道
        private static int iHsession; //播放的session号
       ...
       /// <returns></returns>
        ...
        [DllImport("client.dll", EntryPoint = "HIKS_Stop", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.StdCall)]
        ...
        #region 流媒体模式
        public int StartLiveVideo(string dvsnum, int channel, string strLinkInfo, IntPtr playHandle)
        {
            //if (_isPlaying)
            //{
            //    if ((_dvsNum == dvsnum) && (_dvsChannel == channel))
            //        return 0;
            //    else
        ...
        private int intRet;
        //public int StartLiveVideoLink(string dvsnum, int channel, string strDVRIP, ushort wDVRPort, string strUserName, string strPwd, IntPtr playHandle)
        //{
        //    if (_isPlaying)
        //    { 
        ...
        //    lpPreviewInfo.hPlayWnd = playHandle;//预览窗口
        //    if (dev.byIPChanNum == 0)
        //    {
        ...
        //    else
        //    {
        //        lpPreviewInfo.lChannel = iChannelNum[channel - 1];
        //        //SaveNetLog("播放通道A" + lpPreviewInfo.lChannel.ToString() + "物理通道：" + channel.ToString());
        //    } 
        ...
        //    //if (channel > 16)
        //    //{
        //    //    lpPreviewInfo.lChannel = channel + 16;
        //    //}
        //    //else
        //    //{
        //    //    lpPreviewInfo.lChannel = channel;
        ...
        //            if (ClientVideo._strAlarmTime!="")//141210 海康NVR直接保存录像到客户端上
        //            {
        //                string iniFile = System.Windows.Forms.Application.StartupPath + "\\System.ini";//必须要使用完整的文件名
        //                Ini.IniFile ini = new Ini.IniFile(iniFile);
        //                string strPathDir = ini.IniReadValue("ATDU设置", "RecordPath");
        //                //C:\SHIKE_Video\0026\20131212204654\002601_01_20131212204658_12.mp4
        //                string strRecFileDir = strPathDir + "\\" + dvsnum.Substring(0, 4) + "\\" + ClientVideo._strAlarmTime+"\\";
        //                Directory.CreateDirectory(strRecFileDir);
        //                string sVideoFileName = strRecFileDir  + dvsnum + "_01_" + System.DateTime.Now.ToString("yyyyMMddHHmmss") + "_12.mp4";
        //                HikClientSDK.NET_DVR_MakeKeyFrame(_intDVRHwd, channel);
        //                //开始录像 
        //                if (!NET_DVR_SaveRealData(intRet, sVideoFileName))
        //                {

        //                }
        //                ClientVideo._strAlarmTime = "";
        //            }
        //        }
        //    return intRet;
        //}
        ...
        public bool StopLiveVideoLink()
        {
            try
            {
                ...
                _dvsChannel = 0;
                NET_DVR_CloseSound();
                if (NET_DVR_StopRealPlay(intRet))
                {
        ...
        {
            //FileStream fs = new FileStream("aaa.mp4", FileMode.Append);

            //byte[] data = new byte[ilen];   

            ///System.IntPtr ptr=Marshal.AllocHGlobal(1);
            ///Marshal.WriteByte(pdata, data);   

            //Marshal.Copy(pdata, data);

            //fs.Write(data, 0, ilen);
            //fs.Close();

            return 0;
            //if (OnNotify != null) OnNotify(sid,opt,param1,param2);//向上层提交事件
        }
        ...
        public int intHIKS_PTZControl(int ucommand, int iparam1, int iparam2, int iparam3, int iparam4)
        {
            if (_isPlaying)
            {
                return HIKS_PTZControl(iHsession, ucommand, iparam1, iparam2, iparam3, iparam4);
            }
            else
                return -1;

        }
        public int intHIKS_GetVideoParams(ref int ibri, ref int icon, ref int isat, ref int ihue)
        {
            if (_isPlaying)
            {
        ...
        public int intHIKS_SetVideoParams(int ibri, int icon, int isat, int ihue)
        {
            if (_isPlaying)
            {
                return HIKS_SetVideoParams(iHsession, ibri, icon, isat, ihue);
            }
            else
                return -1;
        }

        public int intHIKS_OpenSound(bool bExclusive)
        {
            if (_isPlaying)
            {
        ...
        public int intHIKS_CloseSound()
        {
            if (_isPlaying)
            {
                if (HIKS_CloseSound(iHsession) == 0)
                {
                    _isOpenSound = false;
                    return 0;
                }
        ...
        public int intHIKS_StartTalk(string strDVRIP, ushort wDVRPort, string strUserName, string strPwd)
        {
            //try
            //{
        ...
        //101204
        public int intHIKS_StopTalk()
        {
            try
            {
                if (_isLoginDVR)
                    if (_isTalking)
                        if (NET_DVR_StopVoiceCom(_intTalkHwd))
                        {
                            NET_DVR_Logout(_intDVRHwd);
                            _isLoginDVR = false;
                            _intDVRHwd = -1;
        ...
        public int intHIKS_SetVolume(UInt16 volume)
        {
            if (_isPlaying)
            {
        ...
        //100125
        public int intHIKS_SetTalkVolume(UInt16 volume)
        {
            if (_isPlaying)
            {
                if (NET_DVR_SetVoiceComClientVolume(_intTalkHwd, volume))
                    return 0;
                else
                    return -1;
            }
